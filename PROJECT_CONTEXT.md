...
8. **Inviolable Production & Test Environment Separation**:
   ...
   - Test suites must clean up all generated test records using cascade-safe foreign-key ordering (auth user deletion cascading doctors before clinic deletion) and assert that post-test user counts return to pre-test baseline.
